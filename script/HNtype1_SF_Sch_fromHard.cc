  ...
  for(ev.toBegin();!ev.atEnd();++ev){
    ...
    for(int i=0;i<gens.size();i++){
      ...
      if(gens[i].isHardProcess()){
        ...
        else if(gens[i].pdgId()==9900012||gens[i].pdgId()==9900014) hard_HN=&gens[i]; 
      }
      if(gens[i].pdgId()==9900012||gens[i].pdgId()==9900014) last_HN=&gens[i];
      ...
      if(gens[i].fromHardProcessFinalState()){
        if(abs(gens[i].pdgId())==11||abs(gens[i].pdgId())==13) leptons.push_back(&gens[i]);
      }
    }
    
    for(int i=0;i<hard_partons.size();i++){
      if(abs((hard_partons.at(i)->mother())->pdgId())==24||(hard_partons.at(i)->mother())->pdgId()==9900012||(hard_partons.at(i)->mother())->pdgId()==9900014) N_partons.push_back(hard_partons.at(i));
